    ...
    >
      ...
      >
        <div className="flex items-center justify-between px-5 py-4 flex-shrink-0" style={{ borderBottom: '1px solid #EEF3F7' }}>
          <h2 className="font-bold text-[15px]" style={{ color: '#2F4156' }}>{initial ? 'Edit Category' : 'New Category'}</h2>
        ...
        <div className="p-5 space-y-4 overflow-y-auto">
          <div>
            <label className="text-xs font-semibold uppercase tracking-wider mb-2 block" style={{ color: '#8EA7B5' }}>Name</label>
            <input value={name} onChange={e => setName(e.target.value)} placeholder="e.g. Faasle BTS" className="cs-input" autoFocus />
          </div>

          <div>
            <label className="text-xs font-semibold uppercase tracking-wider mb-2 block" style={{ color: '#8EA7B5' }}>Color</label>
          ...
          <div>
            <label className="text-xs font-semibold uppercase tracking-wider mb-2 block" style={{ color: '#8EA7B5' }}>Description</label>
          ...
          <div>
            <label className="text-xs font-semibold uppercase tracking-wider mb-2 block" style={{ color: '#8EA7B5' }}>Rules</label>
          ...
          <div>
            <label className="text-xs font-semibold uppercase tracking-wider mb-2 block" style={{ color: '#8EA7B5' }}>Shot Types</label>
...
function CategoryCard({ cat, count, onEdit, onDelete, onClick }: {
  ...
  return (
    <motion.div layout initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }}
      ...
      style={{ background: '#FFFFFF', border: '1px solid #E6EDF2', boxShadow: '0 1px 3px rgba(47,65,86,0.05)', transition: 'box-shadow .2s, transform .2s' }}
      onClick={onClick}
    ...
    >
      ...
      <div className="p-5">
        <div className="flex items-start justify-between gap-3 mb-3">
          <div className="flex items-center gap-2.5 min-w-0">
            ...
            <div className="min-w-0">
              <p className="font-bold truncate" style={{ color: '#2F4156', fontSize: 15 }}>{cat.name}</p>
              {cat.description && <p className="text-xs truncate" style={{ color: '#8EA7B5' }}>{cat.description}</p>}
            </div>
          </div>
          <div className="flex gap-1 opacity-0 group-hover:opacity-100 transition-opacity flex-shrink-0">
            <button onClick={e => { e.stopPropagation(); onEdit() }} className="p-1.5 rounded-lg transition-colors" style={{ color: '#8EA7B5' }}
              onMouseEnter={e => { (e.currentTarget as HTMLElement).style.background = '#F0F6FA' }} onMouseLeave={e => { (e.currentTarget as HTMLElement).style.background = 'transparent' }}>
            ...
            <button onClick={e => { e.stopPropagation(); onDelete() }} className="p-1.5 rounded-lg transition-colors" style={{ color: '#8EA7B5' }}
              onMouseEnter={e => { (e.currentTarget as HTMLElement).style.background = '#FCEEEE'; (e.currentTarget as HTMLElement).style.color = '#B04A4A' }} onMouseLeave={e => { (e.currentTarget as HTMLElement).style.background = 'transparent'; (e.currentTarget as HTMLElement).style.color = '#8EA7B5' }}>
        ...
        <div className="space-y-3">
          <div>
            <p className="text-[10px] font-bold uppercase tracking-wider mb-1" style={{ color: '#A0B8C6' }}>Rules</p>
            <p className="text-xs leading-relaxed" style={{ color: cat.rules ? '#567C8D' : '#C8D9E6', display: '-webkit-box', WebkitLineClamp: 3, WebkitBoxOrient: 'vertical', overflow: 'hidden' } as React.CSSProperties}>
              {cat.rules || 'No rules set yet'}
            </p>
          </div>
          <div>
            <p className="text-[10px] font-bold uppercase tracking-wider mb-1" style={{ color: '#A0B8C6' }}>Shot Types</p>
            <p className="text-xs leading-relaxed" style={{ color: cat.shotTypes ? '#567C8D' : '#C8D9E6', display: '-webkit-box', WebkitLineClamp: 2, WebkitBoxOrient: 'vertical', overflow: 'hidden' } as React.CSSProperties}>
    ...
    <div className="min-h-screen" style={{ background: '#F7F9FB' }}>
      <div className="sticky top-0 z-30" style={{ background: 'rgba(247,249,251,0.94)', backdropFilter: 'blur(12px)', borderBottom: '1px solid #E6EDF2' }}>
        <div className="flex items-center justify-between px-6 py-4 max-w-5xl mx-auto">
          <div>
            <h1 className="font-bold text-[18px]" style={{ color: '#2F4156' }}>Content Categories</h1>
            <p className="text-xs mt-0.5" style={{ color: '#8EA7B5' }}>Define each content type, its rules, and the shots to use</p>
          </div>
      ...
      <div className="max-w-5xl mx-auto px-6 py-6">
        {categories.length === 0 ? (
          <div className="flex flex-col items-center py-32 gap-3 text-center">
            <p className="text-sm font-semibold" style={{ color: '#2F4156' }}>No categories yet</p>
