    ...
    <div className="glossy-card cursor-pointer group" onClick={onClick}>
      <div className="relative" style={{ minHeight: 180 }}>
        {cover ? (
          ...
          <div className="w-full flex items-center justify-center" style={{ background: 'linear-gradient(160deg, #567C8D 0%, #C8D9E6 100%)', minHeight: 180 }}>
        ...
        {item.screenshots?.length > 0 && (
          <span className="absolute top-3 right-3 text-[10px] font-bold px-2 py-1 rounded-full" style={{ background: 'rgba(255,255,255,0.85)', color: '#2F4156' }}>
      ...
      <div className="px-4 pt-3 pb-4 bg-white">
        {item.howToUse && (
          <p className="mb-3 leading-relaxed" style={{ fontSize: 12, color: '#6B8FA3', display: '-webkit-box', WebkitLineClamp: 2, WebkitBoxOrient: 'vertical', overflow: 'hidden' } as React.CSSProperties}>
            {item.howToUse}
          </p>
        )}
...
export default function InspirationsPage() {
  ...
  return (
    <div className="min-h-screen" style={{ background: '#F5EFEB' }}>
      <div className="sticky top-0 z-30" style={{ background: 'rgba(245,239,235,0.94)', backdropFilter: 'blur(12px)', WebkitBackdropFilter: 'blur(12px)', borderBottom: '1px solid #D0DDE6' }}>
        <div className="flex items-center justify-between px-5 py-4 max-w-5xl mx-auto">
          <h1 className="font-bold text-[18px]" style={{ color: '#2F4156' }}>Inspirations</h1>
          <div className="flex items-center gap-3">
            <div className="flex items-center gap-2 px-3 py-2 rounded-xl" style={{ background: '#FFFFFF', border: '1.5px solid #D0DDE6', minWidth: 180 }}>
              ...
              <input value={search} onChange={e => setSearch(e.target.value)} placeholder="Search inspirations…" className="flex-1 bg-transparent outline-none text-sm" style={{ color: '#2F4156' }} />
              {search && (
                <button onClick={() => setSearch('')}>
                  <svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="#8EA7B5" strokeWidth="2.5" strokeLinecap="round"><line x1="18" y1="6" x2="6" y2="18"/><line x1="6" y1="6" x2="18" y2="18"/></svg>
                </button>
              )}
            </div>
        ...
        {categories.length > 0 && (
          <div className="px-5 pb-3 max-w-5xl mx-auto flex items-center gap-2 overflow-x-auto" style={{ scrollbarWidth: 'none' }}>
            <button onClick={() => setFilterCat('')} className="flex-shrink-0 px-3 py-1.5 rounded-full text-xs font-semibold transition-all"
              style={{ background: !filterCat ? '#2F4156' : 'rgba(200,217,230,0.3)', color: !filterCat ? '#FFFFFF' : '#567C8D', border: `1px solid ${!filterCat ? '#2F4156' : '#D0DDE6'}` }}>All</button>
            {categories.map(cat => (
              <button key={cat} onClick={() => setFilterCat(prev => prev === cat ? '' : cat)} className="flex-shrink-0 px-3 py-1.5 rounded-full text-xs font-semibold capitalize transition-all"
                style={{ background: filterCat === cat ? '#567C8D' : 'rgba(200,217,230,0.3)', color: filterCat === cat ? '#FFFFFF' : '#567C8D', border: `1px solid ${filterCat === cat ? '#567C8D' : '#D0DDE6'}` }}>{cat}</button>
            ))}
          </div>
        )}
      </div>
        ...
        {filtered.length === 0 ? (
          <div className="flex flex-col items-center py-32 gap-3 text-center">
            <div className="w-14 h-14 rounded-2xl flex items-center justify-center" style={{ background: '#E8F0F5' }}>
              <svg width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="#567C8D" strokeWidth="1.5" strokeLinecap="round"><path d="M9 18h6"/><path d="M10 22h4"/><path d="M12 2a7 7 0 015 11.95V17a1 1 0 01-1 1H8a1 1 0 01-1-1v-3.05A7 7 0 0112 2z"/></svg>
            </div>
            <p className="text-sm font-semibold" style={{ color: '#2F4156' }}>{inspirations.length === 0 ? 'No inspirations yet' : 'No results'}</p>
            <p className="text-xs" style={{ color: '#8EA7B5' }}>{inspirations.length === 0 ? 'Paste a reel or any link, note how you’ll use it, and add reference shots.' : 'Try clearing the filter.'}</p>
