      ...
      >
        ...
        >
          <button
            ...
            style={{
              background: !filterStatus ? '#33475B' : 'rgba(200,217,230,0.3)',
              color:      !filterStatus ? '#FFFFFF'  : '#516F90',
              border:     `1px solid ${!filterStatus ? '#33475B' : '#DFE3EB'}`,
            }}
          >
            All
          </button>
          {STATUSES.map(st => (
            <button
              ...
              style={{
                background: filterStatus === st ? '#516F90' : 'rgba(200,217,230,0.3)',
                color:      filterStatus === st ? '#FFFFFF'  : '#516F90',
                border:     `1px solid ${filterStatus === st ? '#516F90' : '#DFE3EB'}`,
              }}
            >
        ...
        {categories.length > 0 && (
          <div className="px-5 pb-3 max-w-5xl mx-auto flex items-center gap-2 overflow-x-auto" style={{ scrollbarWidth: 'none' }}>
            <button
              ...
              className="flex-shrink-0 px-3 py-1.5 rounded-full text-xs font-semibold transition-all"
              style={{ background: !filterCat ? '#33475B' : 'rgba(200,217,230,0.3)', color: !filterCat ? '#FFFFFF' : '#516F90', border: `1px solid ${!filterCat ? '#33475B' : '#DFE3EB'}` }}
            >
              All categories
            </button>
            {categories.map(c => {
              ...
              return (
                <button
                  ...
                  style={{ background: active ? `${c.color}1A` : 'rgba(200,217,230,0.3)', color: active ? c.color : '#516F90', border: `1px solid ${active ? `${c.color}55` : '#DFE3EB'}` }}
                >
