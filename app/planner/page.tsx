  ...
  return (
    <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}
      ...
      <motion.div initial={{ y: 26, opacity: 0, scale: 0.98 }} animate={{ y: 0, opacity: 1, scale: 1 }} exit={{ y: 26, opacity: 0, scale: 0.98 }}
        ...
        style={{ background: '#FFFFFF', border: '1px solid #D0DDE6', boxShadow: '0 24px 64px rgba(47,65,86,0.25)', maxHeight: '90vh' }}>
        <div className="flex items-center justify-between px-5 py-4 flex-shrink-0" style={{ borderBottom: '1px solid #EEF3F7' }}>
          <div>
            <h2 className="font-bold text-[15px]" style={{ color: '#2F4156' }}>{entry ? 'Edit plan' : 'Plan a post'}</h2>
            <p className="text-xs mt-0.5" style={{ color: '#8EA7B5' }}>{prettyDate(dateKey)}</p>
          </div>
          <button onClick={onClose} style={{ color: '#8EA7B5' }} className="transition-colors hover:text-[#2F4156]">
        ...
        <div className="p-5 space-y-4 overflow-y-auto">
              ...
              {categories.map(c => {
                ...
                return (
                  <button key={c.id} type="button" onClick={() => setCategoryId(c.id)}
                    ...
                    style={{ background: active ? `${c.color}1A` : '#F5F8FA', color: active ? c.color : '#567C8D', border: `1.5px solid ${active ? c.color : '#E0EAF0'}` }}>
          ...
          <div>
            <label className="text-xs font-semibold uppercase tracking-wider mb-2 block" style={{ color: '#8EA7B5' }}>Note</label>
            <textarea value={note} onChange={e => setNote(e.target.value)} rows={2} placeholder="Optional details…" className="cs-input resize-none" />
          </div>

          {scripts.length > 0 && (
            <div>
              <label className="text-xs font-semibold uppercase tracking-wider mb-2 block" style={{ color: '#8EA7B5' }}>Link a script</label>
          ...
          <div>
            <label className="text-xs font-semibold uppercase tracking-wider mb-2 block" style={{ color: '#8EA7B5' }}>Status</label>
            <div className="flex gap-2">
              {(['planned', 'posted'] as const).map(st => (
                <button key={st} type="button" onClick={() => setStatus(st)}
                  className="flex-1 py-2 rounded-xl text-xs font-semibold capitalize transition-all"
                  style={{ background: status === st ? '#2F4156' : '#F5F8FA', color: status === st ? '#FFF' : '#567C8D', border: `1.5px solid ${status === st ? '#2F4156' : '#E0EAF0'}` }}>
                  {st}
                </button>
              ))}
            </div>
        ...
        <div className="flex gap-3 px-5 py-4 flex-shrink-0" style={{ borderTop: '1px solid #EEF3F7' }}>
          {entry && (
...
export default function PlannerPage() {
  ...
  if (!mounted) return <div className="min-h-screen" style={{ background: '#F7F9FB' }} />
  ...
  return (
    <div className="min-h-screen" style={{ background: '#F7F9FB' }}>
      <div className="sticky top-0 z-30" style={{ background: 'rgba(247,249,251,0.94)', backdropFilter: 'blur(12px)', borderBottom: '1px solid #E6EDF2' }}>
        <div className="flex items-center justify-between px-6 py-4 max-w-5xl mx-auto">
          <div>
            <h1 className="font-bold text-[18px]" style={{ color: '#2F4156' }}>Planner</h1>
            <p className="text-xs mt-0.5" style={{ color: '#8EA7B5' }}>{monthCount} {monthCount === 1 ? 'post' : 'posts'} planned this month</p>
          </div>
          <div className="flex items-center gap-2">
            <button onClick={() => shift(-1)} className="w-8 h-8 rounded-lg flex items-center justify-center transition-colors" style={{ border: '1px solid #E0EAF0', color: '#567C8D', background: '#FFF' }}
              onMouseEnter={e => (e.currentTarget as HTMLElement).style.background = '#F0F6FA'} onMouseLeave={e => (e.currentTarget as HTMLElement).style.background = '#FFF'}>
              <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5" strokeLinecap="round"><polyline points="15 18 9 12 15 6"/></svg>
            </button>
            <span className="text-sm font-bold text-center" style={{ color: '#2F4156', minWidth: 140 }}>{MONTHS[view.m]} {view.y}</span>
            <button onClick={() => shift(1)} className="w-8 h-8 rounded-lg flex items-center justify-center transition-colors" style={{ border: '1px solid #E0EAF0', color: '#567C8D', background: '#FFF' }}
              onMouseEnter={e => (e.currentTarget as HTMLElement).style.background = '#F0F6FA'} onMouseLeave={e => (e.currentTarget as HTMLElement).style.background = '#FFF'}>
      ...
      <div className="max-w-5xl mx-auto px-6 py-6">
          ...
          <div className="flex items-center gap-3 flex-wrap mb-4">
            {categories.map(c => (
              <div key={c.id} className="flex items-center gap-1.5">
                <span className="rounded-full" style={{ width: 8, height: 8, background: c.color }} />
                <span className="text-xs font-medium" style={{ color: '#567C8D' }}>{c.name}</span>
              </div>
            ))}
          </div>
        )}

        <div className="rounded-[18px] overflow-hidden" style={{ background: '#FFFFFF', border: '1px solid #E6EDF2', boxShadow: '0 1px 3px rgba(47,65,86,0.05)' }}>
          {/* Weekday header */}
          <div className="grid grid-cols-7" style={{ borderBottom: '1px solid #EEF3F7' }}>
            {WEEKDAYS.map(w => (
              <div key={w} className="px-2 py-2.5 text-center text-[11px] font-bold uppercase tracking-wider" style={{ color: '#A0B8C6' }}>{w}</div>
            ))}
          </div>
          ...
          <div className="grid grid-cols-7">
            {cells.map((d, i) => {
              if (d === null) return <div key={i} style={{ minHeight: 104, background: '#FBFCFD', borderRight: (i % 7 !== 6) ? '1px solid #F2F6F9' : 'none', borderBottom: '1px solid #F2F6F9' }} />
              ...
              return (
                <div key={i} className="group p-1.5 flex flex-col gap-1 transition-colors"
                  style={{ minHeight: 104, borderRight: (i % 7 !== 6) ? '1px solid #F2F6F9' : 'none', borderBottom: '1px solid #F2F6F9', cursor: 'pointer' }}
                  ...
                  <div className="flex items-center justify-between">
                    <span className="text-xs font-semibold flex items-center justify-center rounded-full"
                      style={{ width: 22, height: 22, color: isToday ? '#FFF' : '#2F4156', background: isToday ? '#2F4156' : 'transparent' }}>{d}</span>
                    <button onClick={e => { e.stopPropagation(); setModal({ dateKey: key }) }}
                      className="opacity-0 group-hover:opacity-100 transition-opacity w-5 h-5 rounded-md flex items-center justify-center" style={{ color: '#A0B8C6' }}>
                    ...
                    {dayPlans.map(p => {
                      ...
                      return (
                        <button key={p.id} onClick={e => { e.stopPropagation(); setModal({ dateKey: key, entry: p }) }}
                          ...
                          <span className="text-[10px] font-semibold truncate block" style={{ color: '#2F4156' }}>
                            {p.title || c?.name || 'Post'}
                          </span>
                        </button>
                      )
        ...
        {categories.length === 0 && (
          <p className="text-center text-xs mt-4" style={{ color: '#A0B8C6' }}>
            Add some <button className="font-semibold underline" style={{ color: '#567C8D' }} onClick={() => router.push('/categories')}>content categories</button> first.
          </p>
        )}
      </div>
